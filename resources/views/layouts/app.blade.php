...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="bg-slate-50 font-sans text-slate-800 antialiased selection:bg-slate-900 selection:text-white">
        @php
            $assignableEntities = auth()->user()?->isSuperAdmin() 
                ? \App\Models\Entity::query()->where('is_active', true)->orderBy('name')->get() 
                : auth()->user()?->entities()->where('is_active', true)->orderBy('name')->get();
        @endphp
        ...
        <div class="min-h-screen flex">
            ...
            <main class="flex-1 lg:pl-64 flex flex-col min-h-screen transition-all duration-300">
                <!-- Header: Desktop Only (Mobile header is now in navigation.blade.php) -->
                <header class="sticky top-0 z-20 hidden lg:block bg-white/70 backdrop-blur-xl border-b border-slate-200/50">
                    <div class="mx-auto flex h-16 w-full items-center justify-between gap-3 px-8">
                        ...
                        @if(isset($assignableEntities) && $assignableEntities->isNotEmpty())
                            <div x-data="{ switcherOpen: false }" class="relative">
                                <button type="button" @click="switcherOpen = !switcherOpen" class="inline-flex h-9 items-center gap-2 rounded-full border border-slate-200/80 bg-white px-3.5 text-sm font-medium text-slate-700 shadow-sm transition hover:bg-slate-50 hover:shadow">
                                    ...
                                    <span class="max-w-40 truncate sm:inline">{{ $activeEntity?->name ?? 'No entity selected' }}</span>
                ...
                <!-- Content Area with adaptive padding for mobile bottom nav -->
                <div class="mx-auto w-full max-w-7xl flex-1 p-4 sm:p-6 lg:p-8 pb-32 lg:pb-8">
                    @if (session('status'))
